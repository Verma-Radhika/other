...
export default {
  ...
  computed: {
          ...
          if (name.includes("shine")) {
            if (shine.push(parameter) === 1)
              features.push({
                ...
                name: "Shine",
                clear: () =>
                  shine.forEach((parameter) =>
                    lipstickParams.reset(parameter.name)
                  ),
              });
    ...
    groups: (vm) =>
      vm.features.reduce((r, a) => ((r[a.group] ??= []).push(a), r), {}),
  },
  methods: {
    serialize() {
      const {
      ...
      for (const [name, { enabled, color, strength }] of Object.entries(skin))
        if (enabled || strength > 0)
          (dump.skin ??= {})[name] = color || strength;

      if (softlight.strength > 0) dump.softlight = softlight.strength;

      if (teethWhitening.strength > 0)
        dump.teethWhitening = teethWhitening.strength;
  ...
  template: /* HTML */ `
    ...
    >
      ...
      <p v-if="empty" class="mb-2 mt-2 bnb-features__no-features">
        No features selected
      </p>
      <div
        class="is-flex is-flex-grow-1 is-flex-direction-column pt-2 bnb-features__list"
      >
        <section
          v-for="(features, name) in groups"
          class="mb-4 bnb-features__list-item"
        >
