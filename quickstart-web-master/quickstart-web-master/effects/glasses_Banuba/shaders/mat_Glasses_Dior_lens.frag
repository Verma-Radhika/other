...
BNB_DECLARE_SAMPLER_2D(12, 13, tex_refl);
...
void main()
{
  ...
  /*  mat3 matr = mat3 (0.96, 0.0, 0.25,
    ...
    vec3 refl = 1.0*BNB_TEXTURE_2D(BNB_SAMPLER_2D(tex_refl), refl_uv ).xyz*(1.-roughness);
    bnb_FragColor = vec4(l2g(color) +refl * F ,pow(opacity,1.5)*1.0);
    bnb_FragColor *=1.0;
}
